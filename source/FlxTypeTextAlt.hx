...
class FlxTypeTextAlt extends FlxText
{
	...
	/**
	 * the delay for certain characters
	 */
    public var slowcharDelay:Float = 0.08;
    ...
    public var slowedcharacters:Array<String> = [".","?","!"];

    public function getCharactersRemoved(remove:Bool = false)
    {
        var leTimer:Float = _timer;
        var index:Int = 0;
		while (leTimer >= (slowedcharacters.contains(_finalText.substr(_length + index, 1)) ? slowcharDelay : delay)) {
			leTimer -= (slowedcharacters.contains(_finalText.substr(_length + index, 1)) ? slowcharDelay : delay);
            index++;
        }
        if (remove)
			_timer = leTimer;
    ...
    public function timerHasPassed()
    {
		return _timer >= (slowedcharacters.contains(_finalText.substr(_length, 1)) ? slowcharDelay : delay);
    }
