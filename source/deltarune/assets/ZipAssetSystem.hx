...
class ZipAssetSystem implements IAssetSystem
{
    ...
    public function new(file:Input, ?root:String, decompress:Bool = false) {
		entries = new Map<String, Entry>();
		directories =  new Array<String>();
		for (entry in Reader.readZip(file)) {
            entries.set(entry.fileName, entry);
            var entryPath = new Path(entry.fileName);
            if (!directories.contains(entryPath.dir)) directories.push(entryPath.dir);
        }
        if (decompress) decompressAll();
        if (root != null) this.root = root;
    }

    public function decompressAll() {
        for (entry in entries) Tools.uncompress(entry);
    }
